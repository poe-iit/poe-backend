...
class Sys:
    ...
    def validate_connection(self, index, set_validity):
        self.c_lock.acquire()
        self.connections[index][0] = set_validity
        self.c_lock.release()
    ...
    def start_client(self, client, address):
        ...
        # Alternatively, instead of timestamps, one can use random num generated by poe_sys
        checking_validity = False

        while checking_validity:
            if get_ident() in self.connections:
                if self.connections[get_ident()][0] == False:
